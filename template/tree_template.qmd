...
ds <- d %>% filter(individual_id == params$tree_id)
ds_species <- d %>% filter(common_name == params$common_name)
```
...
if(nrow(ds) > 0) {
  ...
  leaf_keywords <- c("Colored leaves", "Increasing leaf size", "Leaves")
...
#      theme_minimal() +
      facet_grid(phenophase_description~year(observation_date),scales="free_y")+
